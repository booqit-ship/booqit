...
const CartWidget: React.FC<CartWidgetProps> = ({
  ...
  return <Drawer>
      <DrawerTrigger asChild>
        <Button variant="outline" size="sm" className="relative bg-white border-2 border-booqit-primary text-booqit-primary hover:bg-booqit-primary/10 shadow-lg h-10 w-10 p-0 flex-shrink-0">
          <ShoppingCart className="h-4 w-4" />
          <Badge className="absolute -top-1 -right-1 h-4 w-4 rounded-full p-0 flex items-center justify-center bg-red-500 text-white text-xs font-bold">
      ...
      <DrawerContent className="max-h-[80vh]">
        ...
        <div className="p-4 overflow-y-auto flex-1">
          <div className="space-y-3 mb-6">
            {selectedServices.map(service => <Card key={service.id} className="border border-booqit-primary/20">
                <CardContent className="p-4">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {onRemoveService && <Button variant="ghost" size="sm" onClick={() => onRemoveService(service)} className="h-6 w-6 p-0 text-red-500 hover:text-red-700 hover:bg-red-50">
                          <X className="h-4 w-4" />
                        </Button>}
                    </div>
                  </div>
