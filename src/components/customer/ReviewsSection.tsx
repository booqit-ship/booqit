...
const ReviewsSection: React.FC<ReviewsSectionProps> = ({
  ...
  const fetchUserBookings = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('bookings').select('id').eq('user_id', userId).eq('merchant_id', merchantId).eq('status', 'completed');
  ...
  const fetchReviews = async () => {
    try {
      ...
      const {
        data: reviewsData,
        error: reviewsError
      } = await supabase.from('reviews').select('*').in('booking_id', bookingIds).order('created_at', {
        ascending: false
      });
      ...
      for (const review of reviewsData || []) {
        ...
        const {
          data: profileData
        } = await supabase.from('profiles').select('name, avatar_url').eq('id', review.user_id).single();
  ...
  const getBookingIdsForMerchant = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from('bookings').select('id').eq('merchant_id', merchantId).eq('status', 'completed');
    if (error) {
  ...
  const renderStars = (rating: number, interactive = false, onRatingChange?: (rating: number) => void) => {
    return <div className="flex">
        {[1, 2, 3, 4, 5].map(star => <Star key={star} className={`h-5 w-5 ${star <= rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'} ${interactive ? 'cursor-pointer hover:text-yellow-400' : ''}`} onClick={interactive && onRatingChange ? () => onRatingChange(star) : undefined} />)}
      </div>;
  };
  ...
  return <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <h3 className="text-lg font-righteous font-light">Reviews</h3>
          <span className="text-gray-500 font-poppins">({reviews.length})</span>
        </div>
        {reviews.length > 0 && <div className="flex items-center space-x-2">
      ...
      {userId && <Card>
          <CardContent className="p-4">
            ...
            {!hasCompletedBooking ? <Alert variant="warning">
                <AlertCircle className="h-4 w-4" />
                <AlertDescription className="font-poppins">
                  You need to complete a booking with this salon before you can leave a review.
                </AlertDescription>
              </Alert> : !userReview && !isEditing ? <Button onClick={() => setIsEditing(true)} className="w-full font-poppins">
                Write a Review
              </Button> : isEditing ? <div className="space-y-4">
                <div>
                ...
                <div>
                  ...
                  <Textarea value={editingReview} onChange={e => setEditingReview(e.target.value)} placeholder="Share your experience..." rows={3} className="font-poppins" />
                </div>
              ...
              </div> : userReview ? <div className="space-y-3">
                <div className="flex items-center justify-between">
                ...
                {userReview.review && <p className="text-gray-600 font-poppins">{userReview.review}</p>}
      ...
      <div className="space-y-4">
        {reviews.length > 0 ? reviews.filter(review => review.user_id !== userId) // Don't show user's own review in the list
      .map(review => <Card key={review.id}>
                <CardContent className="p-4">
                  <div className="flex items-start space-x-3">
                    <div className="flex-shrink-0">
                      {review.profiles?.avatar_url ? <img src={review.profiles.avatar_url} alt={review.profiles?.name || 'User'} className="w-10 h-10 rounded-full" /> : <div className="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center">
                    ...
                    <div className="flex-1">
                      ...
                      {review.review && <p className="text-gray-600 mt-2 font-poppins">{review.review}</p>}
                    </div>
                  </div>
                </CardContent>
            ...
            {hasCompletedBooking && <p className="text-sm text-gray-400 mt-1 font-poppins">Be the first to leave a review!</p>}
          </div>}
      </div>
    </div>;
};
