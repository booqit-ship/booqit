...
const CalendarManagementPage: React.FC = () => {
  ...
  const [weekStart, setWeekStart] = useState<Date>(startOfWeek(getCurrentDateIST(), {
    weekStartsOn: 1
  }));
  ...
  const goToToday = () => {
    ...
    setWeekStart(startOfWeek(todayIST, {
      weekStartsOn: 1
    }));
  };
  if (!merchantId) {
  ...
  return <div className="container mx-auto px-4 py-6 pb-20 md:pb-6">
      ...
      <WeekCalendar weekDays={weekDays} selectedDate={selectedDate} onDateSelect={setSelectedDate} onNavigateWeek={navigateWeek} onGoToToday={goToToday} appointmentCounts={appointmentCounts} />

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        <div className="lg:col-span-2">
          <BookingsList date={selectedDate} bookings={bookings} isLoading={loading} onStatusChange={handleStatusChange} />
        </div>

        <div className="space-y-4 md:space-y-6">
          <HolidayManager merchantId={merchantId} holidays={holidays} isLoading={holidaysLoading} onDeleteHoliday={handleDeleteHoliday} onHolidayAdded={fetchHolidays} />

          <StylistAvailabilityWidget merchantId={merchantId} selectedDate={selectedDate} onAvailabilityChange={() => {
          fetchBookings();
          fetchHolidays();
        }} />
        </div>
