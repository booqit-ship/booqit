...
-- Fix the trigger function to be completely null-safe and prevent constraint violations
...
BEGIN
  -- Ensure we always have a notification body to prevent constraint violations
  IF TG_OP = 'INSERT' THEN
    ...
    -- Format datetime consistently with fallbacks
    BEGIN
    ...
    notification_body := COALESCE(customer_name, 'Customer') || ' has booked ' || 
                        COALESCE(service_name, 'a service') || ' for ' || 
                        COALESCE(formatted_datetime, 'scheduled time');

    -- Only insert notification log if we have a valid merchant user_id and non-null body
    IF merchant_user_id IS NOT NULL AND notification_body IS NOT NULL AND trim(notification_body) != '' THEN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    -- Format datetime for updates with error handling
    BEGIN
      booking_datetime := NEW.date::timestamp + NEW.time_slot::interval;
      formatted_datetime := to_char(booking_datetime, 'Mon DD, YYYY') || ' at ' || 
                           to_char(booking_datetime, 'HH12:MI AM');
    ...
    IF NEW.status = 'confirmed' AND OLD.status != 'confirmed' THEN
      ...
      IF NEW.user_id IS NOT NULL THEN
        ...
        IF notification_body IS NOT NULL AND trim(notification_body) != '' THEN
          INSERT INTO notification_logs (user_id, title, body, type)
            VALUES (
              NEW.user_id,
              notification_title,
    ...
    ELSIF NEW.status = 'cancelled' AND OLD.status != 'cancelled' THEN
      ...
      IF NEW.user_id IS NOT NULL THEN
        notification_title := '❌ Booking Cancelled';
        notification_body := 'Your appointment at ' || COALESCE(merchant_name, 'the salon') || 
          ' for ' || COALESCE(formatted_datetime, 'scheduled time') || ' has been cancelled.';
...
-- Also fix the booking reminder function to be null-safe
  ...
  IF NEW.status = 'confirmed' AND NEW.user_id IS NOT NULL THEN
    ...
    BEGIN
      ...
      IF book_time > now() + INTERVAL '25 hours' THEN -- Only for bookings >24h away
        ...
        -- Ensure reminder_body is not null
