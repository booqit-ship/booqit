...
-- Fix the get_available_slots_with_ist_buffer function to properly use total_duration from bookings
...
BEGIN
  ...
  LOOP
    ...
    WHILE slot_time + (p_service_duration || ' minutes')::INTERVAL <= merchant_close_time LOOP
      ...
      -- CRITICAL FIX: Check for overlapping bookings using TOTAL_DURATION from bookings table
      IF EXISTS (
